...
func validateIndexSplitOptions(ctx sessionctx.Context, tblInfo *model.TableInfo, indexInfo *model.IndexInfo, splitOpt *ast.SplitOption) (*SplitIndexRegionOpt, error) {
	...
	minValue, _, err := index.GenIndexKey(ctx.GetSessionVars().StmtCtx, values, math.MinInt64, nil)
	if err != nil {
		return nil, err
	}
	...
	maxValue, _, err := index.GenIndexKey(ctx.GetSessionVars().StmtCtx, values, math.MinInt64, nil)
	if err != nil {
		return nil, err
	}
	...
	if bytes.Compare(minValue, maxValue) >= 0 {
		return nil, errors.Errorf("Pre-split index %v min value should less than the max value", indexInfo.Name)
	}
...
type SplitIndexRegionOpt struct {
	...
	Max        []byte
	Num        int
	ValueLists [][]byte // use index key directly.
}

func longestCommonPrefix(s1, s2 []byte) int {
	i, l := 0, len(s1)
	if len(s2) < len(s1) {
		l = len(s2)
	}
...
func getDiffBytes(startIdx int, min, max []byte) []byte {
	l := len(min)
	if len(max) < len(min) {
		l = len(max)
	}
...
func getValuesList(min, max []byte, num int) [][]byte {
	comPrefixIdx := longestCommonPrefix(min, max)
	startIdx := comPrefixIdx + 1
	diff := getDiffBytes(startIdx, min, max)

	fmt.Printf("split index \n common prefix: %x\n \nmin: %x\nmax: %x\n-----------------------\n", min[:startIdx], min, max)
