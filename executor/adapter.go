...
func (a *ExecStmt) handleNoDelay(ctx context.Context, e Executor, isPessimistic bool) (handled bool, rs sqlexec.RecordSet, _ Executor, err error) {
	...
	if toCheck.Schema().Len() == 0 {
		...
		if isPessimistic {
			...
			if updateExec, ok := e.(*UpdateExec); ok {
				logutil.BgLogger().Info("----- after handlePessimisticDML --0------")
				updateExec.GetFKChecks()
			}
...
func (a *ExecStmt) handlePessimisticDML(ctx context.Context, e Executor) (_ Executor, err error) {
	...
	defer func() {
		...
		if update, ok := e.(*UpdateExec); ok {
			logutil.BgLogger().Info("----- in handlePessimisticDM deferL --3------" + fmt.Sprintf("%#v", e))
			update.GetFKChecks()
		}
	}()
	for {
		...
		if err != nil {
			...
			logutil.BgLogger().Warn("handle lock err", zap.Error(err))
			e, err = a.handlePessimisticLockError(ctx, err)
			if err != nil {
		...
		if _, ok := e.(*UpdateExec); ok {
			logutil.BgLogger().Info("----- after handleNoDelayExecutor --0------")
		}
		if err == nil {
			if updateExec, ok := e.(*UpdateExec); ok {
				logutil.BgLogger().Info("----- after handleNoDelayExecutor --1------")
				updateExec.GetFKChecks()
			}
			return e, nil
		}
		if updateExec, ok := e.(*UpdateExec); ok {
			logutil.BgLogger().Info("----- after handleNoDelayExecutor --2------")
			updateExec.GetFKChecks()
		}
		e, err = a.handlePessimisticLockError(ctx, err)
		if err != nil {
