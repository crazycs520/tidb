...
func (e *inspectionResultRetriever) retrieve(ctx context.Context, sctx sessionctx.Context) ([][]types.Datum, error) {
	...
	if e.instanceToStatusAddress == nil {
		...
		ctx,stmt, err := exec.ParseWithParams(ctx, "select instance,status_address from information_schema.cluster_info;")
		if err == nil {
...
func (configInspection) inspectDiffConfig(ctx context.Context, sctx sessionctx.Context, filter inspectionFilter) []inspectionResult {
	...
	var rows []chunk.Row
	exec := sctx.(sqlexec.RestrictedSQLExecutor)
	ctx,stmt, err := exec.ParseWithParams(ctx, "select type, `key`, count(distinct value) as c from information_schema.cluster_config where `key` not in (%?) group by type, `key` having c > 1", ignoreConfigKey)
	if err == nil {
		rows, _, err = exec.ExecRestrictedStmt(ctx, stmt)
	}
	if err != nil {
	...
	generateDetail := func(tp, item string) string {
		var rows []chunk.Row
		ctx,stmt, err := exec.ParseWithParams(ctx, "select value, instance from information_schema.cluster_config where type=%? and `key`=%?;", tp, item)
		if err == nil {
			rows, _, err = exec.ExecRestrictedStmt(ctx, stmt)
		}
		if err != nil {
...
func (c configInspection) inspectCheckConfig(ctx context.Context, sctx sessionctx.Context, filter inspectionFilter) []inspectionResult {
	...
	for _, cas := range cases {
		...
		ctx,stmt, err := exec.ParseWithParams(ctx, sql.String())
		if err == nil {
			rows, _, err = exec.ExecRestrictedStmt(ctx, stmt)
		}
		if err != nil {
...
func (c configInspection) checkTiKVBlockCacheSizeConfig(ctx context.Context, sctx sessionctx.Context, filter inspectionFilter) []inspectionResult {
	...
	var rows []chunk.Row
	exec := sctx.(sqlexec.RestrictedSQLExecutor)
	ctx,stmt, err := exec.ParseWithParams(ctx, "select instance,value from information_schema.cluster_config where type='tikv' and `key` = 'storage.block-cache.capacity'")
	if err == nil {
		rows, _, err = exec.ExecRestrictedStmt(ctx, stmt)
	}
	if err != nil {
	...
	ctx,stmt, err = exec.ParseWithParams(ctx, "select instance, value from metrics_schema.node_total_memory where time=now()")
	if err == nil {
		rows, _, err = exec.ExecRestrictedStmt(ctx, stmt)
	}
	if err != nil {
...
func (versionInspection) inspect(ctx context.Context, sctx sessionctx.Context, filter inspectionFilter) []inspectionResult {
	...
	var rows []chunk.Row
	// check the configuration consistent
	ctx,stmt, err := exec.ParseWithParams(ctx, "select type, count(distinct git_hash) as c from information_schema.cluster_info group by type having c > 1;")
	if err == nil {
		rows, _, err = exec.ExecRestrictedStmt(ctx, stmt)
	}
	if err != nil {
